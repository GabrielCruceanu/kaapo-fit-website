import { RouteContext, Route } from '../lib/docs/page';

const getRouteContext = (routes: Route[], currentRoute?: Route, ctx: RouteContext = {}): RouteContext => {
    ...
    for (let i = 0; i < routes?.length; i += 1) {
        ...
        if (ctx.route) {
            ctx.nextRoute = parent && i === 0 ? { ...route, title: `${parent.title}: ${route.title}` } : route;
 ...
 */
const useDocsRoute = (routes: Route[], currentRoute?: Route, ctx: RouteContext = {}): RouteContext => {
